...
// 本檔的 JNI / lsplant 輔助函式比照 homefeed.cpp 的實作；homefeed 有
...
// settle 0：AigcCloud 的 URL 在首次使用雲端功能時解析並快取，雖無開機競態，
// 仍比照 homefeed 盡早裝好 hook，避免任何先於 hook 的 region 讀取。
...
void *mediaEditorWorker(void *opaque) {
    ...
    do {
        ...
        if (!application) {
            LOGW("MediaEditor: application unavailable, skip");
            break;
        }

        loader = makeDexLoader(env, application);
        hooker_cls = loader
                ? loadClassWith(env, loader, "jrc.mmedit.RegionHooker")
                : nullptr;
        if (!hooker_cls) {
            LOGW("MediaEditor: hooker class unavailable, safe no-op");
            break;
        }

        // 目標 shape 保護：只驗證 framework 的 SystemProperties.get(String)
        // 存在（不 pin app 版本，理由見 RegionHooker 類註）。
        ...
        if (!g_lsplant_bytes) {
            LOGW("MediaEditor: liblsplant payload missing, safe no-op");
            break;
        }
        ...
        if (!lsplant_init || !lsplant_hook || !lsplant_is_hooked) {
            LOGW("MediaEditor: lsplant symbols missing");
            break;
        }

        auto *resolver = new ArtResolver();  // 跟進程同壽命，不釋放
        if (!resolver->init()) {
            LOGW("MediaEditor: libart symbol table unavailable");
            break;
        }
        ...
        if (!lsplant_init(env, info)) {
            LOGW("MediaEditor: lsplant Init failed");
            break;
        }
        ...
        // 目標是 BOOTCLASSPATH 類，直接 FindClass；AigcCloud 經反射呼叫的
        // 正是這個 1-arg overload，反射路徑不可能被 AOT inline，hook 必中。
        ...
        if (clearException(env, "SystemProperties.get") || !target_cls || !target_mid) {
            LOGW("MediaEditor: prop target missing, safe no-op");
        ...
        if (clearException(env, "lsplant Hook") || !backup) {
            LOGW("MediaEditor: lsplant Hook returned null");
            break;
        }
        ...
        LOGI("MediaEditor region hook installed (IsHooked=%d)", hooked ? 1 : 0);
...
void mmeditStartEditor(JavaVM *vm) {
    if (!vm) return;
    pthread_t thread;
    if (pthread_create(&thread, nullptr, mediaEditorWorker, vm) == 0) {
        pthread_detach(thread);
    } else {
        LOGW("MediaEditor: unable to start worker");
    }
}
